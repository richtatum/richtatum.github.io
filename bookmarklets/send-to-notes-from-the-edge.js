...
function sendToNotesFromTheEdge(){
   ...
   if(wDescription){description=wDescription.content.replace(/\s+/g,' ').trim();}
   else{
      const ogDescription=document.querySelector('meta[property="og:description"]').replace(/\s+/g,' ').trim();
      const twitterDescription=document.querySelector('meta[name="twitter:description"]').replace(/\s+/g,' ').trim();
      if(ogDescription){description=ogDescription.content.replace(/\s+/g,' ').trim();}
      else if(twitterDescription){description=twitterDescription.content.replace(/\s+/g,' ').trim();}
      }
   ...
   if(description!==""){params.append('entry.1626307794', description);}
